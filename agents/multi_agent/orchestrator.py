...
# -------------------------------------------------------------
# TOOL BRIDGE — the one place Phase 4 imports Phase 3 code
#
# The specialists reuse Phase 3's three tools (same Redis key
# conventions, same DIM queries, same Weaviate search) rather
# than forking them — the tools ARE the platform's data access
# layer for agents; two copies would drift. single_agent/tools.py
# does `import config` internally and expects the agent phase's
# settings; because this phase's config.py re-exports every
# Phase 3 constant (see config.py), the already-cached
# sys.modules["config"] — ours — satisfies it. The sys.path
# insert only makes the `tools` module FINDABLE; it's removed
# immediately after so later imports can't accidentally resolve
# into the Phase 3 folder (tools.py handles its own retrieval/
# import dance internally, including the config-name swap).
#
# Specialists receive their tools via constructor injection from
# here instead of importing this bridge themselves — one bridge,
# one owner, and every specialist file stays free of cross-phase
# import mechanics.
# -------------------------------------------------------------
_SINGLE_AGENT_DIR = os.path.abspath(
    os.path.join(os.path.dirname(__file__), "..", "single_agent")
)
sys.path.insert(0, _SINGLE_AGENT_DIR)
try:
    ...
    sys.path.remove(_SINGLE_AGENT_DIR)
