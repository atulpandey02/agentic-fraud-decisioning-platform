...
# Make Phase 2's retrieval/ modules importable from here. This
# project has no setup.py / installable package structure — every
# phase folder assumes itself as the working directory — so
# cross-folder imports need an explicit path insert rather than
# a normal package import.
#
# The path insert alone is NOT enough, though — this phase and
# retrieval/ BOTH have a module named `config`, and Python caches
# modules by name in sys.modules. By the time this file runs,
# sys.modules["config"] is already THIS phase's config.py
# (imported by agent.py before it imports tools), so
# weaviate_client's own `from config import WEAVIATE_COLLECTION_NAME`
# would resolve against the agent's config — which deliberately
# carries only connection values, none of Phase 2's chunking/
# embedding/collection settings — and die with ImportError before
# the graph is ever built. The fix: pop the agent's entry out of
# sys.modules for exactly the duration of these imports, letting
# retrieval/config.py claim the name, then restore. Safe because
# `from config import X` binds values at import time — once
# FraudKnowledgeBase and Embedder finish importing, they hold
# their own references and never look the name up again. The
# honest alternative — renaming one of the two config modules —
# would break the established one-config-py-per-phase convention
# across every file in that phase, for the sake of this one
# import site.
_RETRIEVAL_DIR = os.path.abspath(
    os.path.join(os.path.dirname(__file__), "..", "..", "retrieval")
)
sys.path.insert(0, _RETRIEVAL_DIR)
_agent_config_entry = sys.modules.pop("config", None)
try:
    from weaviate_client import FraudKnowledgeBase   # noqa: E402
    from embedder import Embedder                     # noqa: E402
finally:
    # Put the world back exactly as it was: the name `config` must
    # keep meaning THIS phase's settings for every later import in
    # this process, and retrieval/ comes off sys.path so no future
    # fresh import can accidentally resolve into Phase 2's folder.
    sys.modules.pop("config", None)
    if _agent_config_entry is not None:
        sys.modules["config"] = _agent_config_entry
    sys.path.remove(_RETRIEVAL_DIR)
